...
uint8_t DHT22_Read(DHT22_Data *data)
{
   ...
   delay_ms1(2);
    ...
    data->dht_humidity = rawHumidity / 10.0f;
    if (rawTemp & 0x8000)
        data->dht_temperature = -((rawTemp & 0x7FFF) / 10.0f); // ici on convertis la temperature mesuree en valeur reel
    else
        data->dht_temperature = rawTemp / 10.0f;
